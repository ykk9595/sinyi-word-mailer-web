...
const ACCESS_SESSION_KEY = "sinyi-property-sheet-access";
...
function houseNoFromUrl(value) {
  const url = new URL(value);
  if (!["sinyi.com.tw", "www.sinyi.com.tw"].includes(url.hostname)) {
    throw new Error("請輸入 sinyi.com.tw 的信義房屋網址");
  }
  const match = url.pathname.match(/\/buy\/house\/([0-9a-z]+)(?:\/|$)/i);
  if (!match) throw new Error("網址中找不到有效的信義房屋物件編號");
...
function valueAfter(lines, label) {
  ...
  return index < 0 ? "" : text(lines[index + 1]);
}
...
function parseLayout(value) {
  ...
  return {
    rooms: numberFromText(source.match(/(\d+)房/)?.[1]),
    livingRooms: numberFromText(source.match(/(\d+)廳/)?.[1]),
    bathrooms: numberFromText(source.match(/(\d+)衛/)?.[1])
  };
}
...
function parseSinyiMarkdown(markdown, requestedHouseNo) {
  ...
  const basic = sectionLines(lines, "基本資料", "建物詳情");
  const detail = sectionLines(lines, "建物詳情", "收合物件詳情");
  if (!basic.length) throw new Error("信義房屋回應中找不到基本資料");
  ...
  const hasParking = Boolean(parkingText) && !/^(--|無|否)$/.test(parkingText);
  const managementText = valueAfter(basic, "管理費");
  const featureSection = sectionLines(lines, "物件特色", "周邊生活機能");
  const excludedFeatures = new Set([
    "機能", "特色", "屋況", "近公園", "近市場", "近學校", "近超市", "2D/3D看屋",
    "房間皆有窗", "有陽台", "有裝潢", "有電梯", "廁所開窗", "前後陽台", "警衛管理",
    "有景觀", "邊間", "無障礙空間"
  ]);
  const descriptions = featureSection
    .filter(line => !excludedFeatures.has(line) && !/^https?:\/\//i.test(line))
    .slice(-5);

  const expert = sectionLines(lines, "本案專家", "基本資料");
  ...
  return {
    ...
    addrSimp: address,
    buildingName: "",
    landShPin: numberFromText(valueAfter(basic, "地坪")),
    buiTotPin: numberFromText(valueAfter(basic, "建坪")),
    buiMPin: numberFromText(valueAfter(basic, "主+陽")),
    buiAuxPin: 0,
    typeCodeName: valueAfter(basic, "類型"),
    useCodeName: valueAfter(detail, "謄本用途"),
    positionName: valueAfter(basic, "建物朝向"),
    mgExpense: numberFromText(managementText),
    mgCode: managementText ? "月繳" : "無",
    parkingSpace: hasParking ? parkingText : "無",
...
function extractFeatureLines(description) {
  return text(description)
    ...
    .filter(Boolean)
    .map(line => Array.from(line).slice(0, 7).join(""))
    .slice(0, 5);
}
...
function normalizeCase(raw) {
  ...
  for (const fallback of ["格局方正好規劃", "採光通風都很好", "交通生活都便利", "社區環境很清幽", "自住置產都適宜"]) {
    if (features.length === 5) break;
